...
class ProductViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if OrderItem.objects.filter(product_id=kwargs['pk']).count() > 0:
            return Response({'error': "Can't delete products as it contains one or more order items."},
...
class CollectionViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if Collection.objects.count('products') > 0:
            return Response({'error': "Can't delete collection as it contains one or more products."},
                            status=status.HTTP_405_METHOD_NOT_ALLOWED)
